...
def read_sales_data():
    ...
    with open(sales_path, "r") as sales_csv:
        ...
        for sale in sales:
            sale.update({"Amount": int(sale["Amount"])})
...
def read_employee_data():
    ...
    """
    sales_path = os.path.join(CURRENT_DIR, "employee_data.csv")
    employee_data = []
    with open(sales_path, "r") as employee_csv:
        employees = csv.DictReader(employee_csv)
        for employee in employees:
            employee.update({"Salary": int(employee["Salary"])})
...
def calculate_median_sales(sales_data):
    ...
    """
    amounts: list[int] = [item["Amount"] for item in sales_data]
    amounts.sort()
    # mid: int = int(len(sales_data) / 2)
    mid: int = len(sales_data) // 2
    return amounts[mid] if len(amounts)%2==1 else (amounts[mid]+amounts[mid-1])/2
...
def calculate_total_salary_expenses(employee_data):
    ...
    """
    return sum([x["Salary"] for x in employee_data])
...
def calculate_median_salary(employee_data):
    ...
    """
    salaries: list[int] = [item["Salary"] for item in employee_data]
    salaries.sort()
    # mid: int = int(len(sales_data) / 2)
    mid: int = len(employee_data) // 2
    return salaries[mid] if len(salaries)%2==1 else (salaries[mid]+salaries[mid-1])/2
...
def find_employee_with_highest_sales(sales_data, employee_data):
    ...
    """
    high = max([item["Amount"] for item in sales_data])
    for item in sales_data:
        if item["Amount"] == high:
            top_employee = item["EmployeeID"]
            break
    for item in employee_data:
        if top_employee == item["EmployeeID"]:
            name = item["Name"]
            dep = item["Department"]
            break
    return name, dep
