...
class UserRepository:
  ...
  def get_one(self, user_id: str) -> dict:
    ...
    if not docSnapshot.exists:
      raise Exception(f"User with id {user_id} doesn't exists in database")
  ...
  def get_user_by_username(self, username: str) -> dict:
    ...
    if len(users) == 0:
      raise Exception(f"User with username: {username} doesn't exists in database")
    
    assert len(users) == 1

    user: DocumentSnapshot = users[0]
    user_ret: dict = user.to_dict()
    user_ret.update({"id": user.id})
    return user_ret